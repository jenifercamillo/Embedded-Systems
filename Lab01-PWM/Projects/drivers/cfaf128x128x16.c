...
// display with an ST7735S controller. This version uses an SSI interface.
// This driver was made to be used with the TivaWare grlib library and was based
// on the Kentec320x240x16_ssd2119_spi driver for the ek-tm4c1294xl board, the
// cfal96x64x16 driver for the dk-tm4c123g board and the Energia Screen_HX8353E
// driver for the ek-tm4c1294xl board.
//
// Modified by Hugo Vieira Neto (12. August 2019)
//
//*****************************************************************************
...
#include "driverlib/pin_map.h"  // PART_TM4C1294NCPDT
...
// Abstraction for delay calls
#define delay(d) SysCtlDelay(d * 10)

//*****************************************************************************
//
// Defines the SSI and GPIO peripherals that are used for the display.
//
//*****************************************************************************
...
//*****************************************************************************
//
// Defines the GPIO pin configurations for the pins that are used for the SSI
// function.
//
//*****************************************************************************
...
#define DISPLAY_SSI_PINS         (DISPLAY_SSI_PIN_CLK | DISPLAY_SSI_PIN_FSS |  \
                                  DISPLAY_SSI_PIN_TX | DISPLAY_SSI_PIN_RX)

//*****************************************************************************
//
// Defines the port and pins for the display reset (RST) signal.
//
//*****************************************************************************
...
//*****************************************************************************
//
// Defines the port and pins for the display chip select (CS) signal.
//
//*****************************************************************************
...
#define DISPLAY_SSI_CLOCK        (SystemCoreClock / 2)
...
//! controller. It can be used for single commands. It will wait for any
//! previous operation to finish, and then copy all the command bytes to the
//! controller. It will not return until the last command byte has been
//! written to the SSI FIFO, but data could still be shifting out to the
//! display controller when this function returns.
//!
//! \return None.
//
//*****************************************************************************
...
cfaf128x128x16WriteData(const uint8_t *data8, uint32_t ndata){		
  ...
  GPIOPinWrite(DISPLAY_CS_PORT, DISPLAY_CS_PIN, DISPLAY_CS_PIN);
} // cfaf128x128x16WriteData

// Writes one command byte and one data byte.
static void 
...
} // cfaf128x128x16WriteRegister

// Sets up the drawing window. Data sent to the display will fill the pixels of 
...
cfaf128x128x16SetWindow(uint16_t x0, uint16_t x1, uint16_t y0, uint16_t y1){
  ...
  // Prepares to write to the display pixel matrix.
  cfaf128x128x16WriteCommand(CM_RAMWR);
} // cfaf128x128x16SetWindow

// Draws a rectangle with the given coordinates and color.
void 
cfaf128x128x16Rect(uint16_t x0, uint16_t x1, uint16_t y0, uint16_t y1,
                   uint32_t color){
  uint32_t t;	
  uint32_t hor_size;
  uint32_t ver_size;
  uint8_t highByte = (0xFF00 & color) >> 8;
  uint8_t lowByte  =  0xFF & color;
...
//Draws a vertical line with the given coordinates, thickness and color
void 
cfaf128x128x16LineVer(uint16_t x, uint16_t y0, uint16_t y1, uint16_t thickness,
                      uint32_t color){
  cfaf128x128x16Rect(x, x + thickness, y0, y1, color);
} // cfaf128x128x16LineVer

//Draws a horizontal line with the given coordinates, thickness and color
void 
cfaf128x128x16LineHor(uint16_t x0, uint16_t x1, uint16_t y, uint16_t thickness,
                      uint32_t color){
  cfaf128x128x16Rect(x0, x1, y, y + thickness, color);
} // cfaf128x128x16LineHor

//Draws a dot with the given coordinates, thickness and color.
void
...
cfaf128x128x16PixelDrawMultiple(void *pvDisplayData, int32_t i32X, int32_t i32Y,
  ...
  // Determinea how to interpret the pixel data based on the number of bits
  // per pixel.
...
//*****************************************************************************
static void
cfaf128x128x16LineDrawH(void *pvDisplayData, int32_t i32X1, int32_t i32X2,
                        int32_t i32Y, uint32_t ui32Value){
  if(i32X1 > i32X2)
    swap_i32(&i32X1, &i32X2);
...
//*****************************************************************************
static void
cfaf128x128x16LineDrawV(void *pvDisplayData, int32_t i32X, int32_t i32Y1,
                        int32_t i32Y2, uint32_t ui32Value){
  if(i32Y1 > i32Y2)
    swap_i32(&i32Y1, &i32Y2);
...
// Sets the orientation for the rect filling
void 
cfaf128x128x16SetOrientation(uint8_t orientation){
  uint8_t data = 0;
  _orientation = orientation & 0x11;
...
} // cfaf128x128x16SetOrientation

// Clears screen with solid color
void 
...
// Initialiazes SSI comunication configuration
void
InitSSICom(void){
  uint32_t initialData = 0;
  
  // Enables the peripherals used by this driver
  ...
  SSIConfigSetExpClk(DISPLAY_SSI_BASE, SystemCoreClock, SSI_FRF_MOTO_MODE_0,
                     SSI_MODE_MASTER, DISPLAY_SSI_CLOCK, 8);
...
cfaf128x128x16Init(void){
  ...
  // Configures display control pins as GPIO output
